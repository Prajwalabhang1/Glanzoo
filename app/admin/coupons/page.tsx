        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {couponList.map((coupon) => {
                    ...
                    return (
                        <div key={coupon.id} className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 hover:shadow-md transition-shadow">
                            <div className="flex items-start justify-between mb-4">
                                ...
                                <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${isActive ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>
                            ...
                            <div className="bg-gradient-to-br from-orange-50 to-amber-50 rounded-lg p-4 mb-4">
                                ...
                                <p className="text-xs text-gray-600 mt-1">{coupon.type === 'PERCENTAGE' ? 'Percentage Off' : 'Flat Discount'}</p>
                                {coupon.minOrder && <p className="text-xs text-gray-600 mt-2">Min. order: ₹{Number(coupon.minOrder).toLocaleString()}</p>}
                            </div>

                            <div className="mb-4">
                                <div className="flex justify-between text-sm mb-1">
                                    <span className="text-gray-600">Usage</span>
                                    <span className="font-medium text-gray-900">{coupon.usageCount} / {coupon.usageLimit || '∞'}</span>
                                </div>
                                <div className="w-full bg-gray-200 rounded-full h-2">
                                    <div className="bg-gradient-to-r from-orange-500 to-amber-500 h-2 rounded-full transition-all" style={{ width: `${Math.min(usagePercent, 100)}%` }} />
                                </div>
                            </div>

                            <div className="flex items-center justify-between text-sm mb-4 pb-4 border-b border-gray-100">
                                <span className="text-gray-600">Expires</span>
                                <span className="font-medium text-gray-900">{new Date(coupon.validUntil).toLocaleDateString()}</span>
                            </div>
            ...
            {couponList.length === 0 && (
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-12 text-center">
                    ...
                    <Link href="/admin/coupons/new">
                        <Button className="bg-gradient-to-r from-orange-500 to-amber-500"><Plus className="w-4 h-4 mr-2" /> Create Coupon</Button>
                    </Link>
                </div>
            )}
        </div>
