...
export async function ProductGrid({ searchParams }: ProductGridProps) {
    ...
    if (priceRange) {
        ...
        conditions.push(gte(products.price, min), lte(products.price, max));
    }

    const orderBy = sort === 'price-low' ? asc(products.price) : sort === 'price-high' ? desc(products.price) : sort === 'popular' ? desc(products.sales) : desc(products.createdAt);
    ...
    const [variantRows, categoryRows] = productIds.length > 0 ? await Promise.all([
        db.select().from(productVariants).where(and(inArray(productVariants.productId, productIds))),
    ...
    const enriched = productRows.map(p => ({
        ...p, images: p.images, category: catMap[p.categoryId] ?? null,
        variants: (varMap[p.id] ?? []).filter((v: any) => v.stock > 0).sort((a: any, b: any) => a.size.localeCompare(b.size)),
    }));
    ...
    return (
        <div>
            <div className="flex items-center justify-between mb-6"><p className="text-gray-600">Showing <span className="font-semibold">{enriched.length}</span> products</p></div>
